...
async function renderPageHandler(
  req,
  res,
  pagename,
  msgOpt,
...
exports.createUserPostController = async (req, res, next) => {
  try {
    ...
    let user = new User({
      ...
      createdAt:
        makeDateHandler().date +
        " " +
        makeDateHandler().month +
        " " +
        makeDateHandler().year,
    });
...
exports.updateUserPostController = async (req, res, next) => {
  try {
    let { name, username, email, phone, password, gender, securityPassword } =
      req.body;
    let file = req.file;
    ...
    if (password.length !== 0) {
      ...
      if (securityPassword.length !== 0) {
        let securityPasswordCheck = await bcrypt.compare(
          securityPassword,
          admin.securityPassword
        );
        if (!securityPasswordCheck) {
          ...
          return renderPageHandler(
            ...
            "createTeacher",
            "fail",
            "Security Password Dosen't Matched",
            null,
            error.mapped()
          );
        }
        ...
        req.flash(
          "fail",
          "Please provied security password if you want to change user password"
        );
